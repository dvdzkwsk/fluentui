 ...
 */ export const defaultErrorMessages = {
    ...
    'component-renders': (testInfo, error)=>{
        ...
        if (requiredProps) {
            const formattedRequiredProps = Object.entries(requiredProps).map(([propName, propValue])=>`  ${propName}: ${propValue}`).join(',' + EOL);
            return getErrorMessage({
                displayName,
    ...
    'consistent-callback-names': (testInfo, invalidProps)=>{
        ...
        return getErrorMessage({
            ...
            details: [
                ...
                testErrorInfo(formatArray(invalidProps))
            ],
            suggestions: [
    ...
    'consistent-callback-args': (testInfo, invalidProps)=>{
        ...
        return getErrorMessage({
            ...
            details: [
                ...
                testErrorInfo(formatErrors(invalidProps))
            ],
            suggestions: [
    ...
    'component-has-static-classname-exported': (testInfo, error, componentClassName, exportName)=>{
        ...
        return getErrorMessage({
            displayName,
            overview: `default className constant  is not exported (${testErrorInfo(exportName)}) in: ${EOL}${testErrorPath(indexFile)}.`,
            suggestions: [
    ...
    'component-has-static-classnames-object-exported': (testInfo, error, exportName)=>{
        ...
        return getErrorMessage({
            displayName,
            overview: `static classNames object is not exported (${testErrorInfo(exportName)}) in: ${EOL}${testErrorPath(indexFile)}.`,
            suggestions: [
    ...
    'component-has-static-classnames-in-correct-format': (testInfo, error, exportName)=>{
        ...
        return getErrorMessage({
            displayName,
            overview: `static classNames object is not formatted correctly (${testErrorInfo(exportName)}) in: ${EOL}${testErrorPath(indexFile)}.`,
            suggestions: [
