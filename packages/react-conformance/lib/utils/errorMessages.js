...
import { EOL } from 'os';
/** Console message colors used in the test. */ export const errorMessageColors = {
...
export function getErrorMessage(params) {
    ...
    if (suggestions) {
        messageParts.push(sectionBackground('Possible solutions:'), suggestions.map((msg, i)=>resolveText(`${i + 1}. ${msg}`)).join(EOL));
    }
    if (error) {
 ...
 */ export function formatErrors(value) {
    return value ? Object.entries(value).map(([propName, error])=>`    ${propName}: ${error.message}`).join(EOL) : 'received undefined';
}
