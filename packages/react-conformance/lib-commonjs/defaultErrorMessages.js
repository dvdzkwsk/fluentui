...
Object.defineProperty(exports, "defaultErrorMessages", {
    enumerable: true,
    get: function() {
...
const defaultErrorMessages = {
    'exports-component': (testInfo, error, exportNames)=>{
        const { componentPath, displayName } = testInfo;
        const { testErrorPath, resolveInfo } = _index.errorMessageColors;
        ...
        // isConformant test.
        return (0, _index.getErrorMessage)({
            displayName,
            overview: `is not exported from:${_os.EOL}${testErrorPath(componentPath)}`,
            details: [
                `The available exports are: ${exportNames.join(', ')}`
            ],
            suggestions: [
                `Make sure that your component's file contains an export for ${resolveInfo(displayName)}.`,
                testInfo.useDefaultExport ? `Make sure your component file has ${resolveInfo(displayName)} as its default export.` : `Check to see if your component uses ${resolveInfo('export default')} and consider enabling ${resolveInfo('useDefaultExport')} in your isConformant test.`
            ],
            error
        });
    },
    'component-renders': (testInfo, error)=>{
        const { displayName, requiredProps } = testInfo;
        const { testErrorInfo, resolveInfo } = _index.errorMessageColors;
        ...
        if (requiredProps) {
            const formattedRequiredProps = Object.entries(requiredProps).map(([propName, propValue])=>`  ${propName}: ${propValue}`).join(',' + _os.EOL);
            return (0, _index.getErrorMessage)({
                displayName,
                overview: "doesn't render successfully.",
                details: [
                    'It currently is receiving the requiredProps:',
                    testErrorInfo(formattedRequiredProps)
                ],
                suggestions: [
                    `Make sure you're including all of the required props to render in isConformant ${resolveInfo('requiredProps')}.`,
        ...
        return (0, _index.getErrorMessage)({
            ...
            suggestions: [
                `Make sure that your are including all of the required props to render in isConformant ` + resolveInfo('requiredProps') + '.',
                `Make sure that your component's implementation contains a valid return statement.`,
                `Check to see if your component works as expected with Enzyme's ${resolveInfo('mount()')}.`
            ],
            error
    ...
    'component-has-displayname': (testInfo, error)=>{
        var _Component_prototype;
        const { componentPath, Component, displayName } = testInfo;
        const { testErrorInfo, testErrorPath, resolveInfo } = _index.errorMessageColors;
        ...
        if (!componentDisplayName || componentDisplayName === 'Styledundefined') {
            return (0, _index.getErrorMessage)({
                displayName,
                overview: `doesn't have a display name in:${_os.EOL}${testErrorPath(componentPath)}`,
                suggestions: [
        ...
        return (0, _index.getErrorMessage)({
            displayName,
            overview: `has an incorrect display name. It received: ${testErrorInfo(componentDisplayName.replace('Styled', ''))}`,
            suggestions: [
                `Make sure the component file contains ${resolveInfo(`${displayName}.displayName = '${displayName}'`)}`
            ],
            error
    ...
    'component-handles-ref': (testInfo, error)=>{
        const { displayName } = testInfo;
        const { resolveInfo } = _index.errorMessageColors;
        ...
        return (0, _index.getErrorMessage)({
            ...
            suggestions: [
                ...
                `If your component uses an "elementRef" prop instead of "ref", add ${resolveInfo(`elementRefName: 'elementRef'`)} to isConformant in your test file.`
            ],
            error
        });
    },
    'component-has-root-ref': (testInfo, error)=>{
        const { displayName } = testInfo;
        const { resolveInfo } = _index.errorMessageColors;
        ...
        return (0, _index.getErrorMessage)({
            ...
            suggestions: [
                ...
                `Check if your component overrides the primary slot, and add ${resolveInfo(`primarySlot`)} to isConformant in your test file.`,
                `Check if your component uses an element ref and add ${resolveInfo(`elementRefName: 'elementRef'`)} to isConformant in your test file.`,
                `Check if your component passes ref to an inner component and add ${resolveInfo(`getTargetElement`)} to isConformant in your test file.`
            ],
            error
        });
    },
    'omits-size-prop': (testInfo, error, sizeValue, appliedToElement)=>{
        const { displayName } = testInfo;
        const { resolveInfo, testErrorName } = _index.errorMessageColors;
        ...
        return (0, _index.getErrorMessage)({
            ...
            details: [
                ...
                (0, _react.prettyDOM)(appliedToElement)
            ],
            suggestions: [
    ...
    'component-handles-classname': (testInfo, error, testClassName, classNames, rootEl)=>{
        const { displayName } = testInfo;
        const { testErrorInfo, resolveInfo, failedError, testErrorName } = _index.errorMessageColors;
        ...
        const debugHTML = elementWithClass ? (0, _react.prettyDOM)(elementWithClass, 50) + '...' : '';
        ...
        return (0, _index.getErrorMessage)({
            ...
            details: [
                `After passing a test className ${testErrorInfo(`"${testClassName}"`)} to ${testErrorName(displayName)} it received:`,
                `    ${failedError(`className="${classNames === null || classNames === void 0 ? void 0 : classNames.join(' ')}"`)}`,
                ...debugHTML ? [
                    '',
                    'Partial HTML:',
                    debugHTML
            ...
            suggestions: [
                ...
                `Make sure that nothing is ${resolveInfo('overwriting')} the className prop ` + `(it should be merged with defaults).`,
                `Make sure that your ${resolveInfo('component is applying')} the className to the root.`
            ],
            error
        });
    },
    'component-preserves-default-classname': (testInfo, error, testClassName, defaultClassName, classNames)=>{
        const { displayName } = testInfo;
        const { testErrorInfo, resolveInfo, failedError, testErrorName } = _index.errorMessageColors;
        ...
        return (0, _index.getErrorMessage)({
            ...
            details: [
                `After passing a test classname ${testErrorInfo(`"${testClassName}"`)} to ${testErrorName(displayName)} it received a className:`,
                `    ${failedError(`className='${classNames === null || classNames === void 0 ? void 0 : classNames.join(' ')}'`)}`,
                '',
                `Since ${testErrorName(displayName)} has a default className "${testErrorInfo(defaultClassName)}" it should have been merged with the user-supplied className.`
            ],
            suggestions: [
                `Check the placement of your className and ensure that it is ${resolveInfo('merged')} with defaults.`,
                `Check if your component passes className to an inner element and add ${resolveInfo('getTargetElement')} ` + 'to isConformant in your test file.'
            ],
            error
        });
    },
    'name-matches-filename': (testInfo, error)=>{
        ...
        return (0, _index.getErrorMessage)({
            ...
            suggestions: [
                `Make sure that your component's filename matches the displayName "${resolveInfo(displayName)}" passed to isConformant`
            ],
            error
        });
    },
    'exported-top-level': (testInfo, error)=>{
        const { displayName, componentPath } = testInfo;
        const { testErrorPath, resolveInfo } = _index.errorMessageColors;
        const indexFile = _path.join((0, _index.getPackagePath)(componentPath), 'src', 'index');
        const exportInfo = testInfo.useDefaultExport ? `export { default as ${displayName} } from './${displayName};` : `export * from './${displayName}';`;
        ...
        return (0, _index.getErrorMessage)({
            displayName,
            overview: `doesn't have a top level export in:${_os.EOL}${testErrorPath(indexFile)}`,
            suggestions: [
                `Make sure that your component's ${resolveInfo('index.ts')} file contains \`${resolveInfo(exportInfo)}\``,
                `Check if your component is internal and consider enabling ${resolveInfo('isInternal')} in your isConformant test.`
            ],
            error
        });
    },
    'has-top-level-file': (testInfo, error)=>{
        const { displayName, componentPath } = testInfo;
        const { testErrorPath, resolveInfo } = _index.errorMessageColors;
        const topLevelFile = _path.join((0, _index.getPackagePath)(componentPath), 'src', displayName);
        // Message Description: Handles scenario where the displayName doesn't match the component's filename.
        //
        ...
        return (0, _index.getErrorMessage)({
            displayName,
            overview: `doesn't have a top level file in:${_os.EOL}${testErrorPath(topLevelFile)}`,
            suggestions: [
    ...
    'kebab-aria-attributes': (testInfo, invalidProps)=>{
        const { displayName } = testInfo;
        const { testErrorName } = _index.errorMessageColors;
        ...
        return (0, _index.getErrorMessage)({
            ...
            suggestions: [
                ...
                testErrorName((0, _index.formatArray)(ariaPropsDiff)),
                "For props that shouldn't be renamed, add them to isConformant `testOptions['"
            ]
        });
    },
    // Note: the original error isn't included in params since the test's final checks are narrowly scoped
    'consistent-callback-names': (testInfo, invalidProps)=>{
        const { displayName } = testInfo;
        const { testErrorInfo, resolveInfo } = _index.errorMessageColors;
        ...
        return (0, _index.getErrorMessage)({
            ...
            details: [
                'These callback(s) need to be renamed:',
                testErrorInfo((0, _index.formatArray)(invalidProps))
            ],
            suggestions: [
                `Rename ${resolveInfo(displayName + `'s`)} callback props to use present tense (no "ed" ending)`,
                `If the name is correct, add the prop to isConformant ${resolveInfo("testOptions['consistent-callback-names'].ignoreProps")}.`
            ]
        });
    },
    'consistent-callback-args': (testInfo, invalidProps)=>{
        const { displayName } = testInfo;
        const { testErrorInfo, resolveInfo } = _index.errorMessageColors;
        return (0, _index.getErrorMessage)({
            displayName,
            ...
            details: [
                'These callback(s) have issues:',
                testErrorInfo((0, _index.formatErrors)(invalidProps))
            ],
            suggestions: [
                `Ensure that ${resolveInfo(displayName + `'s`)} callbacks have two params (an event and data object) and types of arguments are correct.`,
                `If a callback is intended to have a different signature, add the prop to isConformant ${resolveInfo("testOptions['consistent-callback-args'].ignoreProps")}.`
            ]
        });
    },
    'component-has-static-classname': (testInfo, error, componentClassName, classNames)=>{
        const { displayName } = testInfo;
        const { testErrorInfo, resolveInfo, failedError } = _index.errorMessageColors;
        return (0, _index.getErrorMessage)({
            displayName,
    ...
    'component-has-static-classname-exported': (testInfo, error, componentClassName, exportName)=>{
        const { componentPath, displayName } = testInfo;
        const { testErrorInfo, resolveInfo, testErrorPath } = _index.errorMessageColors;
        const indexFile = _path.join((0, _index.getPackagePath)(componentPath), 'src', 'index.ts');
        const constantValue = `export const ${exportName} = "${componentClassName}";`;
        return (0, _index.getErrorMessage)({
            displayName,
            overview: `default className constant  is not exported (${testErrorInfo(exportName)}) in: ${_os.EOL}${testErrorPath(indexFile)}.`,
            suggestions: [
                `Make sure that your component's ${resolveInfo('index.ts')} file ` + `or a file with styles hook exports \`${resolveInfo(constantValue)}\``,
                `If the component is internal, consider enabling ${resolveInfo('isInternal')} in your isConformant test.`
            ],
            error
        });
    },
    'component-has-static-classnames-object-exported': (testInfo, error, exportName)=>{
        const { componentPath, displayName } = testInfo;
        const { testErrorInfo, resolveInfo, testErrorPath } = _index.errorMessageColors;
        const indexFile = _path.join((0, _index.getPackagePath)(componentPath), 'src', 'index.ts');
        return (0, _index.getErrorMessage)({
            displayName,
            overview: `static classNames object is not exported (${testErrorInfo(exportName)}) in: ${_os.EOL}${testErrorPath(indexFile)}.`,
            suggestions: [
                `Make sure that your component's ${resolveInfo('index.ts')} file ` + `or a file with styles hook exports an object of type SlotClassNames<Slots>.`,
                `If the component is internal, consider enabling ${resolveInfo('isInternal')} in your isConformant test.`
            ],
            error
        });
    },
    'component-has-static-classnames-in-correct-format': (testInfo, error, exportName)=>{
        const { componentPath, displayName } = testInfo;
        const { testErrorInfo, resolveInfo, testErrorPath } = _index.errorMessageColors;
        const indexFile = _path.join((0, _index.getPackagePath)(componentPath), 'src', 'index.ts');
        return (0, _index.getErrorMessage)({
            displayName,
            overview: `static classNames object is not formatted correctly (${testErrorInfo(exportName)}) in: ${_os.EOL}${testErrorPath(indexFile)}.`,
            suggestions: [
                `Make sure all classNames are for the form "fui-<ComponentName>__<SlotName>" ` + `or "fui-<ComponentName>" for the root slot`,
                `If the component is internal, consider enabling ${resolveInfo('isInternal')} in your isConformant test.`
            ],
            error
        });
    },
    'component-has-static-classnames': (testInfo, error, componentName, missingClassNames, rootEl)=>{
        const { displayName } = testInfo;
        const { testErrorInfo, failedError, resolveInfo } = _index.errorMessageColors;
        return (0, _index.getErrorMessage)({
            displayName,
            ...
            details: [
                ...
                'Actual HTML:',
                (0, _react.prettyDOM)(rootEl)
            ],
            suggestions: [
                `Ensure that each slot of the component has its corresponding static className.`,
                `If the component is rendered in a portal, add ${resolveInfo(`getTargetElement`)} to isConformant in your test file.`,
                `If the component requires certain props to render all slots, add ${resolveInfo('staticClassNames.props')} to isConformant in your test file. (You can add multiple prop combinations.)`
            ],
            error
        });
    },
    'as-renders-html': (testInfo, error)=>{
        const { displayName } = testInfo;
        const { resolveInfo } = _index.errorMessageColors;
    ...
    'primary-slot-gets-native-props': (testInfo, error)=>{
        const { displayName } = testInfo;
        const { resolveInfo } = _index.errorMessageColors;
        return (0, _index.getErrorMessage)({
            displayName,
            ...
            suggestions: [
                `Make sure you're using the ${resolveInfo('getPartitionedNativeProps')} function to assign the correct native props to the root and primary slots.`,
